...
## 3. Pick the smallest next slice
...
work on</plan>`. Like the completion-promise tag in section 9, this is
harness protocol, not project-facing output — it is exempt from section
6's "never leak loop vocabulary" rule.

## 4. Implement it with strict TDD
...
6. Never write a placeholder, stub, or no-op implementation just to make a
   ...
   blocker (section 7) — do not fake it to get green.
...
## 6. Never let the loop leak into project-facing output
...
never be referenced anywhere except your own end-of-turn report (section 9
...
## 7. If something is blocked, don't thrash — report it
...
## 9. Report, and only promise completion when it's actually true
